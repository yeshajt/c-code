...
bool lookup(string s)
{
    // Use a binary search to check whether the user input is a word

    // Establishes the minimum and maximum index of the list
    int minimum = 0;
    int maximum = WORDS - 1;

    while (minimum <= maximum)
    {
        int middle_index = (minimum + maximum) / 2;

        string middle = dictionary.words[middle_index].letters;

        if (strcmp(s, middle) == 0)
        {
            if (dictionary.words[middle_index].found)
            {
                return false;
            }
            else
            {
                // Marks the word as found in case the user tries it again
                dictionary.words[middle_index].found = true;
                return true;
            }

        }

        // Checks higher letters in case s is alphabetically greater
        else if (strcmp(s, middle) > 0)
        {
            minimum = middle_index + 1;
        }

        // Checks lower letters in case s is alphabetically less
        else
        {
            maximum = middle_index - 1;
        }
    }
...
void scramble(void)
{
    ...
    char placeholder[DIMENSION][DIMENSION];

    // Saves the grid values in the placeholder values
    for (int row = 0; row < DIMENSION; row++)
    {
        for (int column = 0; column < DIMENSION; column++)
        {
            placeholder[row][column] = grid[row][column];
        }
    }

    // Stores the grid values into the new grid locations
    for (int row = 0; row < DIMENSION; row++)
    {
        for (int column = DIMENSION - 1; column >= 0; column--)
        {
            grid[row][DIMENSION - 1 - column] = placeholder[column][row];
        }
    }
}
